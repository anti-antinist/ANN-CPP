...
struct NeuronID {
	...
	NeuronID() = default;
	bool operator==(NeuronID n) { return n.l == this->l && n.n == this->n; }
};

template <typename type> class ANN {
...
template <typename type> ANN<type>::ANN(const std::vector<unsigned int> &layern, const std::vector<std::pair<NeuronID, NeuronID>> &ResWeights, type (*actfuncHIDp)(type), type (*actfuncOUTp)(type)) {
	initialize(layern, ResWeights);
	actfuncHID = actfuncHIDp;
	actfuncOUT = actfuncOUTp;
}

template <typename type> ANN<type>::ANN(std::string filename, bool isBIN, type (*actfuncHIDp)(type), type (*actfuncOUTp)(type)) {
	if(isBIN) {
...
template <typename type> ANN<type>::ANN(const ANN<type> &net) {
...
template <typename type> ANN<type>::~ANN() { layers.clear(); }

template <typename type> void ANN<type>::initialize(const std::vector<unsigned int> &layern, const std::vector<std::pair<NeuronID, NeuronID>> ResWeights) {
...
template <typename type> std::vector<type> ANN<type>::costvec(const std::vector<type> &target, unsigned int l) {
	std::vector<type> out;
	for(unsigned int n = 0; n < layers[l].neurons.size(); n++) {
...
template <typename type> typename ANN<type>::NEURON &ANN<type>::IDtoN(NeuronID nID) { return *&*(layers[nID.l].neurons.begin() + nID.n); }

template <typename type> type ANN<type>::costavg(const std::vector<type> &target) {
...
template <typename type> std::vector<type> ANN<type>::forwardpropagate(const std::vector<type> &input) {
...
template <typename type> void ANN<type>::deserializecsv(std::string filename) {
...
template <typename type> void ANN<type>::serializecsv(std::string filename) {
...
template <typename type> void ANN<type>::deserializebin(std::string filename) {
	...
	while(file.read(reinterpret_cast<char *>(&layerid), sizeof(int)) && file.read(reinterpret_cast<char *>(&neuronid), sizeof(int))) {
		if(last > layerid) {
			break;
		}
		type weight = 0.0f;
...
template <typename type> void ANN<type>::serializebin(std::string filename) {
...
template <typename type> void ANN<type>::resetStructure(const std::vector<int> &layern, const std::vector<std::pair<NeuronID, NeuronID>> &ResWeights) { initialize(layern, ResWeights); }

template <typename type> void ANN<type>::resetStructure(const ANN<type> &net) {
	actfuncHID = net.actfuncHID;
	actfuncOUT = net.actfuncOUT;
	layers.clear();
	for(unsigned int l = 0; l < net.layers.size(); l++) {
...
template <typename type> void ANN<type>::backpropagate(const std::vector<type> &input, const std::vector<type> &target, type learn_rate, bool learn_rate_safety) {
...
template <typename type> void ANN<type>::deleteNeuron(unsigned int lID) {
...
template <typename type> void ANN<type>::addNeuron(unsigned int lID) {
	assert(lID >= 0 && lID < layers.size());
...
template <typename type> void ANN<type>::deleteLayer(unsigned int lID) {
...
template <typename type> void ANN<type>::addLayer(unsigned int lID, unsigned int s) {
...
template <typename type> void ANN<type>::DeleteResidualWeight(NeuronID from, NeuronID to) {
...
template <typename type> void ANN<type>::AddResidualWeight(NeuronID from, NeuronID to, type weight) {
	assert(from.l < layers.size() || to.l < layers.size());
	assert(from.n < layers[from.l].neurons.size() || to.n < layers[to.l].neurons.size());
	assert(from.l < to.l);
...
template <typename type> struct ANN<type>::NEURON {
...
template <typename type> ANN<type>::NEURON::~NEURON() {
	outweights.clear();
	resWeights.clear();
}

template <typename type> void ANN<type>::NEURON::initializeweights() {
	for(type &i : outweights)
		i = 2 * rand() / (type)RAND_MAX;
	bias = 2 * rand() / (type)RAND_MAX;
}

template <typename type> struct ANN<type>::LAYER {
   public:
...
template <typename type> ANN<type>::LAYER::LAYER(unsigned int n, unsigned int curr, unsigned int next_s, ANN<type> *pbelong_to) {
	init(n, curr, next_s, pbelong_to);
	belong_to = pbelong_to;
}

template <typename type> ANN<type>::LAYER::~LAYER() {
	neurons.clear();
	belong_to = nullptr;
}

template <typename type> void ANN<type>::LAYER::init(unsigned int n, unsigned int curr, unsigned int next_s, ANN<type> *pbelong_to) {
	assert(n > 0);
...
template <typename type> void ANN<type>::LAYER::calcActs(type(actfunc)(type)) {
...
template <typename type> struct ANN<type>::ResidualWeight {
...
template <typename type> ANN<type>::ResidualWeight::ResidualWeight(NeuronID fromp, type weightp) {
	from = fromp;
	weight = weightp;
}

template <typename type> class EVO_TRAINER {
   ...
   public:
	...
	EVO_TRAINER(const unsigned int n_of_networks, const ANN<type> &template_ANN, type (*fitnessp)(ANN<type> &net), type mutate_rate);
	~EVO_TRAINER();
	void mutate_generation(bool re_structure, type mutate_rate, type re_structure_constant);
	ANN<type> &best_speciman();
	void re_init(const unsigned int n_of_networks, const std::vector<unsigned int> &structure, const std::vector<std::pair<NeuronID, NeuronID>> &ResWeights, type (*actHID)(type), type (*actOUT)(type), type (*fitnessp)(ANN<type> &net));
	void re_init(const unsigned int n_of_networks, const ANN<type> &template_ANN, type (*fitnessp)(ANN<type> &net), type mutate_rate);
};

template <typename type> EVO_TRAINER<type>::EVO_TRAINER(const unsigned int n_of_networks, const std::vector<unsigned int> &structure, const std::vector<std::pair<NeuronID, NeuronID>> &ResWeights, type (*actHID)(type), type (*actOUT)(type), type (*fitnessp)(ANN<type> &net)) {
	std::srand((clock() + time(NULL)) / 2);
...
template <typename type> EVO_TRAINER<type>::EVO_TRAINER(const unsigned int n_of_networks, const ANN<type> &template_ANN, type (*fitnessp)(ANN<type> &net), type mutate_rate) {
	for(unsigned int nn = 0; nn < n_of_networks; nn++) {
...
template <typename type> EVO_TRAINER<type>::~EVO_TRAINER() {
...
template <typename type> void EVO_TRAINER<type>::mutate_generation(bool re_structure, type mutate_rate, type re_structure_constant) {
	...
	if(re_structure) {
		for(unsigned int nn = 0; nn < networks.size(); nn++) {
			bool to_add_l = int(mutate_rate * double(rand() * re_structure_constant)) % 2;
			if(to_add_l) {
			...
			for(unsigned int l = 1; l < networks[nn]->layers.size() - 1; l++) {
				bool to_add_n = int(mutate_rate * double(rand() * re_structure_constant)) % 2;
				if(to_add_n) {
				...
				for(unsigned int n = 0; n < networks[nn]->layers[l].neurons.size(); n++) {
					bool to_add_r = int(mutate_rate * double(rand() * re_structure)) % 2;
					if(to_add_r) {
...
template <typename type> ANN<type> &EVO_TRAINER<type>::best_speciman() {
...
template <typename type> void EVO_TRAINER<type>::re_init(const unsigned int n_of_networks, const std::vector<unsigned int> &structure, const std::vector<std::pair<NeuronID, NeuronID>> &ResWeights, type (*actHID)(type), type (*actOUT)(type), type (*fitnessp)(ANN<type> &net)) {
	for(auto &network : networks) {
		delete network;
	}
	networks.clear();
...
template <typename type> void EVO_TRAINER<type>::re_init(const unsigned int n_of_networks, const ANN<type> &template_ANN, type (*fitnessp)(ANN<type> &net), type mutate_rate) {
	for(auto &network : networks) {
		delete network;
	}
	networks.clear();
